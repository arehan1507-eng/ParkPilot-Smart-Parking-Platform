...
├── GITHUB_IGNORE_RULES.txt # Rules for the GitHub .gitignore file
...
The project works with the local defaults. To customize them, create local `.env` files using the visible examples included with this upload:

```powershell
Copy-Item server\ENVIRONMENT_EXAMPLE.txt server\.env
Copy-Item client\ENVIRONMENT_EXAMPLE.txt client\.env
```
...
If you use MongoDB Atlas, replace `MONGODB_URI` with your Atlas connection string. Do not commit `.env` files or real secrets to GitHub. After your upload, create a `.gitignore` file on GitHub using `GITHUB_IGNORE_RULES.txt`.
